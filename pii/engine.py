...
def build_analyzer() -> Optional[AnalyzerEngine]:
    ...
    try:
        ...
        # Add enhanced recognizers
        enhanced_recognizers = [
            EnhancedRobustRecognizer("ID", [
                Pattern("SSN_DASHED", r'\b\d{3}-\d{2}-\d{4}\b', 0.9),
                Pattern("SSN_SPACED", r'\b\d{3}\s\d{2}\s\d{4}\b', 0.9),
                Pattern("SSN_RAW", r'\b\d{9}\b', 0.6)
            ]),
            EnhancedRobustRecognizer("PHONE_NUMBER", [
                Pattern("US_PHONE", r'\b\+?1?[-.\s]?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b', 0.8),
                Pattern("INTERNATIONAL_PHONE", r'\b\+\d{1,3}[-.\s]?\d{1,4}[-.\s]?\d{1,4}[-.\s]?\d{1,9}\b', 0.7)
            ]),
            EnhancedRobustRecognizer("ADDRESS", [
                Pattern("FULL_ADDRESS", r'\b\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Court|Ct|Place|Pl|Way|Terrace|Ter)(?:\s+(?:Apt|Apartment|Unit|Suite|Ste|#)\s*[A-Za-z0-9]+)?,\s+[A-Za-z\s]+,\s+[A-Z]{2}\s+\d{5}(?:-\d{4})?\b', 0.9),
                Pattern("STREET_ADDRESS", r'\b\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Court|Ct|Place|Pl|Way|Terrace|Ter)\b', 0.8),
                Pattern("CITY_STATE_ZIP", r'\b[A-Za-z\s]+,\s+[A-Z]{2}\s+\d{5}(?:-\d{4})?\b', 0.85)
            ]),
            EnhancedRobustRecognizer("EMAIL_ADDRESS", [
                Pattern("EMAIL", r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', 1.0)
            ]),
            EnhancedRobustRecognizer("CREDIT_CARD", [
                Pattern("CREDIT_CARD", r'\b\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{1,4}\b', 0.9),
                Pattern("CREDIT_CARD_RAW", r'\b\d{13,19}\b', 0.7)
            ]),
            EnhancedRobustRecognizer("EIN", [
                Pattern("EIN", r'\b\d{2}-\d{7}\b', 0.9),
                Pattern("EIN_RAW", r'\b\d{9}\b', 0.6)
            ]),
    ...
    for i, chunk in enumerate(chunks):
        try:
            results = analyzer.analyze(
                text=chunk,
                entities=["ID", "PHONE_NUMBER", "ADDRESS", "EMAIL_ADDRESS", "CREDIT_CARD", "EIN", "ZIP"],
                language="en"
            )
