...
from config import (
    CONTEXT_KEYWORDS, FALSE_POSITIVE_INDICATORS,
    INVALID_SSN_PATTERNS, INVALID_AREA_CODES,
    CONTEXT_BOOST, FALSE_POSITIVE_PENALTY,
    MIN_CONFIDENCE
)
...
class ContextValidator:
    ...
    def __init__(self):
        ...
        self.false_positive_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(fp) for fp in FALSE_POSITIVE_INDICATORS) + r')\b',
            re.IGNORECASE
        )
        
    def validate_ssn(self, value: str, context: str = "") -> Tuple[bool, float]:
        """
        Validate SSN with context.
        
        Args:
            value: The SSN value detected
        ...
        if value in INVALID_SSN_PATTERNS:
            return False, 0.0
    ...
    def validate_phone(self, value: str, context: str = "") -> Tuple[bool, float]:
        ...
        Args:
            value: The phone number detected
            context: Surrounding text context
        ...
        if us_phone_match:
            ...
            if area_code in INVALID_AREA_CODES:
                ...
                if area_code == '555':
                    ...
                    rest = value.replace(area_code, '').replace('-', '').replace(' ', '')
    ...
    def validate_email(self, value: str, context: str = "") -> Tuple[bool, float]:
        ...
        if any(test in lower_email for test in ['example.com', 'test.com', 'demo.com', 
                                                  'sample.com', 'localhost', '127.0.0.1']):
            return False, 0.0
            
        # Check for noreply/system emails (usually not PII)
        if any(sys in lower_email for sys in ['noreply@', 'no-reply@', 'donotreply@',
                                               'system@', 'admin@', 'root@']):
            confidence_adj = -0.3
        else:
            confidence_adj = 0.0
    ...
    def validate_address(self, value: str, context: str = "") -> Tuple[bool, float]:
        ...
        if any(fake in lower_addr for fake in ['123 main st', '123 fake st', 
                                                '1234 test lane', 'example address']):
            return False, 0.0
    ...
    def validate_entity(self, entity_type: str, value: str, 
                       context: str = "", confidence: float = 1.0) -> Tuple[bool, float]:
        """
