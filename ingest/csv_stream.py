...
class StreamingCSVProcessor:
    ...
    def get_progress(self) -> tuple[int, int | None, float]:
        """Get current progress."""
        if self.total_rows is None:
            self.estimate_total_rows()

        progress_pct = (
            (self.rows_processed / self.total_rows * 100) if self.total_rows else 0
        )
    ...
    def stream_concatenated_text(
        self, chunk_rows: int = 1000, max_chars: int = 10000
    ) -> Iterator[str]:
        """
