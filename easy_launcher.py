    ...
    while True:
        ...
        if path.is_file():
            ...
            try:
                ...
                if file_count > 1000:
                    print(
                        "⚡ Large directory detected - will use optimized processing!"
                    )
            except:
...
def run_background_scan(scan_path, output_path):
    ...
    cmd = ["venv/bin/python", "pii_launcher.py", scan_path, output_path]
    ...
    try:
        ...
        print("   👀 View results anytime:  ./view")
        print(f"   📊 Check scan status:      ps -p {process.pid}")
        print(
            f"   🔄 Watch results update:   watch -n 2 'venv/bin/python -m app.status_cli --out {output_path}'"
        )
        print()
    ...
    try:
        ...
        if return_code == 0:
            ...
            if total_entities > 0:
                ...
                print("\n📊 SCAN SUMMARY:")
                print("   ✅ No personal information detected in your files")
...
def show_results(output_path, success):
    ...
    if success and output_dir.exists():
        ...
        if summary_file.exists():
            ...
            try:
                with open(summary_file) as f:
                    ...
                    rows_with_pii.sort(
                        key=lambda x: int(x.get("total", 0)), reverse=True
                    )
        ...
        print("\n📚 QUICK COMMANDS:")
        print(
            f"   📊 View summary:     venv/bin/python -m app.status_cli --out '{output_path}'"
        )
        print(
            f"   🔍 See actual PII:   venv/bin/python -m app.results_explorer --out '{output_path}'"
        )
        ...
        if choice == "1":
            ...
            try:
                ...
                print(
                    f"💡 Run: venv/bin/python -m app.status_cli --out '{output_path}'"
                )
            ...
            try:
                ...
                print(
                    f"💡 Run: venv/bin/python -m app.results_explorer --out '{output_path}'"
                )
    else:
        print("\n❌ No results to show. Check error messages above.")
        print("\n💡 TROUBLESHOOTING:")
        print("   1. Make sure the path exists and you have permission to read it")
        print("   2. Try running: python setup.py")
...
def main():
    ...
    try:
        ...
        print("👋 You can run this again anytime.")
    except Exception as e:
        print(f"\n❌ Unexpected error: {e}")
