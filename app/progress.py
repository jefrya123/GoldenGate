...
class ProgressTracker:
    ...
    def __init__(self, operation_id: str, total_items: int | None = None):
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            if self.total_items and self.total_items > 0:
                progress_pct = min(
                    (self.processed_items / self.total_items) * 100, 100.0
                )
    ...
    def print_progress(self, prefix: str = "Progress"):
        ...
        if stats["total_items"]:
            ...
            print(
                f"\r{prefix}: {progress_bar} {stats['progress_percentage']:.1f}% "
            ...
            print(
                f"\r{prefix}: {stats['processed_items']} items | {bytes_str} @ {rate_str} "
    ...
    def finish(self):
        ...
        print(
            f"   📊 Processed: {stats['processed_items']} items in {self._format_duration(stats['elapsed_seconds'])}"
        )
        print(f"   📁 Files: {stats['files_processed']}")
        print(f"   💾 Data: {self._format_bytes(stats['bytes_processed'])}")
        print(
            f"   🔍 Entities: {stats['entities_found']} total ({stats['controlled_entities']} controlled, {stats['noncontrolled_entities']} non-controlled)"
        )

        if stats["items_per_second"] > 0:
            print(
                f"   ⚡ Rate: {stats['items_per_second']:.1f} items/sec, {self._format_bytes(stats['bytes_per_second'])}/sec"
            )
