...
Examples:
  ...
  %(prog)s --out ./results --interval 5 --limit 50
    ...
    print(
        f"Filters: min-sev={args.min_sev or 'ALL'}, only-suspect={args.only_suspect}, limit={args.limit}"
    )
    ...
    try:
        while True:
            ...
            print(
                f"Output: {out_dir} | Refresh: {args.interval}s | Filters: min-sev={args.min_sev or 'ALL'}, only-suspect={args.only_suspect}, limit={args.limit}"
            )
            print()
