#!/usr/bin/env python3
"""
Snapshot status CLI - displays PII detection results from summary.csv
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Snapshot status view of PII detection results",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s --out ./results
  %(prog)s --out ./results --only-suspect --min-sev MEDIUM
  %(prog)s --out ./results --limit 50
        """,
    )
    ...
    parser.add_argument(
        "--min-sev",
        choices=["NONE", "LOW", "MEDIUM", "CRITICAL"],
        help="Minimum severity level to show",
    )
    parser.add_argument(
        "--only-suspect", action="store_true", help="Show only files with suspected PII"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    if not summary_path.exists():
        print(f"Error: summary.csv not found in {out_dir}", file=sys.stderr)
        sys.exit(1)
