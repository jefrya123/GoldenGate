...
class ResumeManager:
    ...
    def load_checkpoint(self, file_path: Path) -> Optional[Dict[str, Any]]:
        """
    ...
    def list_pending_checkpoints(self) -> List[Dict[str, Any]]:
        ...
        for checkpoint_file in self.resume_dir.glob("checkpoint_*.json"):
            try:
                ...
                if file_path.exists() and self._is_checkpoint_valid(checkpoint_data, file_path):
                    pending.append({
                        'file_path': str(file_path),
...
class SmartResumeScanner:
    ...
    def scan_with_resume(self, file_path: Path, scanner_func, operation_id: str = None) -> Dict[str, Any]:
        """
