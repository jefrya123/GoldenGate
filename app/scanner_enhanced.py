...
class EnhancedScanner:
    ...
    def __init__(self, out_dir: Path, extensions: set[str] | None = None):
        """
    ...
    def should_skip_file(self, file_path: Path) -> tuple[bool, str]:
        ...
        try:
            ...
            if size > MAX_FILE_SIZE:
                return (
                    True,
                    f"File too large: {size / (1024*1024):.1f} MB (limit: {MAX_FILE_SIZE / (1024*1024):.0f} MB) - use large file scanner",
                )
        except:
    ...
    def scan_file_with_stats(self, file_path: Path) -> FileSummary | None:
        """
    ...
    def _print_progress(self):
        ...
        print(
            f"\r  Progress: {self.stats['files_scanned']} files scanned, "
    ...
    def collect_files(self, scan_path: Path) -> list[Path]:
        ...
        for root, dirs, filenames in os.walk(scan_path):
            ...
            if FEATURES["smart_filtering"]:
                dirs[:] = [
                    d for d in dirs if not self.should_skip_directory(root_path / d)
                ]
    ...
    def scan_batch(self, files: list[Path]) -> list[FileSummary | None]:
        """
    ...
    def scan(self, scan_path: Path) -> dict[str, Any]:
        ...
        print(
            f"  Multi-threading: {'Enabled' if FEATURES['multi_threading'] else 'Disabled'}"
        )
        print(
            f"  Smart filtering: {'Enabled' if FEATURES['smart_filtering'] else 'Disabled'}"
        )
