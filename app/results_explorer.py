...
def export_to_csv(data: List[Dict[str, Any]], output_path: Path):
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        # Enhanced headers
        headers = [
            'File Path', 'Filename', 'Severity', 'Controlled', 'NonControlled', 
            'Total', 'File Size (bytes)', 'Modified Date', 'Top Entity Types'
        ]
...
def interactive_explorer(out_dir: Path):
    ...
    if files_with_pii:
        ...
        for i, row in enumerate(files_with_pii, 1):
            ...
            print(f"     Severity: {severity} | Entities: {total} ({controlled} Controlled, {noncontrolled} NonControlled)")
    ...
    if choice == "1":
        output_path = out_dir / f"pii_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        export_to_csv(data, output_path)
        print(f"✅ Exported to: {output_path}")
    
    elif choice == "2":
        output_path = out_dir / f"pii_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
Examples:
  ...
  %(prog)s --export-json ./results            # Export to JSON
    ...
    parser.add_argument('--discover', action='store_true', 
                       help='Discover output directories')
    ...
    if args.export_csv:
        ...
        output_path = Path(f"pii_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        export_to_csv(data, output_path)
        print(f"✅ Exported to: {output_path}")
        return
    ...
    if args.export_json:
        ...
        output_path = Path(f"pii_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        export_to_json(data, output_path)
        print(f"✅ Exported to: {output_path}")
        return
    ...
    if args.out:
        ...
        if not output_dirs:
            ...
            for i, out_dir in enumerate(output_dirs, 1):
                print(f"{i}. {out_dir}")
            choice = input("Select directory (1-{}): ".format(len(output_dirs)))
            try:
