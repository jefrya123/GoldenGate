...
def format_entity(entity: dict[str, Any]) -> str:
    ...
    if context_left or context_right:
        lines.append(
            f"  📄 Context: ...{context_left} [{entity.get('value', '')}] {context_right}..."
        )
...
def export_entities_to_csv(
    entities: list[dict[str, Any]], output_path: Path, filename: str
):
...
def show_file_details(out_dir: Path, filename: str):
    ...
    print(
        f"📊 Summary: {file_info.get('controlled', 0)} Controlled, {file_info.get('noncontrolled', 0)} NonControlled, {file_info.get('total', 0)} Total"
    )
...
Examples:
  ...
  %(prog)s --out ./results --file document.txt --export-csv
    ...
    parser.add_argument(
        "--export-csv", action="store_true", help="Export entities to CSV"
    )
    ...
    try:
        ...
        if csv_file.exists():
            ...
            with open(csv_file, encoding="utf-8") as f:
                ...
                for row in reader:
                    file_path = row.get("file", "")
                    if (
                        file_path.endswith(args.file)
                        or Path(file_path).name == args.file
                    ):
                        target_file_path = file_path
                        file_info = row
                        break
        ...
        if args.export_csv:
            ...
            output_path = (
                out_dir
                / f"entities_{args.file}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
